...
assert.equal(recipeNutritionFromIngredients(recipe).sodiumMg, null);
assert.equal(recipeNutritionFromIngredients(recipe).ironMg, null);
assert.equal(recipeNutritionFromIngredients({ ...recipe, ingredients: [...recipe.ingredients, { optional: false, count: 1, groceryItem: { gramsPerCount: 3, nutritionPer100g: {} } }] }).calories, null);
assert.equal(recipeNutritionFromIngredients({ ...recipe, servings: 0 }).calories, null);
assert.equal(recipeNutritionFromIngredients({ ...recipe, servings: null }).calories, 15);
assert.equal(recipeNutritionFromIngredients({ ...recipe, ingredients: [{ optional: false, weightGrams: 100, groceryItem: { nutritionPer100g: { calories: 0 } } }] }).calories, 0);
assert.equal(recipeNutritionFromIngredients({ ...recipe, ingredients: [{ optional: false, groceryItem: { nutritionPer100g: { calories: 100 } } }] }).calories, null);
