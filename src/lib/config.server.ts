...
import "server-only";
const must = (k:string)=>{const v=process.env[k]; if(!v) throw new Error(`Missing env: ${k}`); return v;};
const int=(k:string,d:number)=>{const n=parseInt(process.env[k]??"",10); return Number.isFinite(n)?n:d;};
export const CFG_SERVER = {
  DATABASE_URL: must("DATABASE_URL"),
  STRIPE_SECRET_KEY: must("STRIPE_SECRET_KEY"),
  STRIPE_WEBHOOK_SECRET: must("STRIPE_WEBHOOK_SECRET"),
  PAYPAL_SECRET: must("PAYPAL_SECRET"),
