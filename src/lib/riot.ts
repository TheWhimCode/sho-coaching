...
if (!RIOT_KEY) {
  console.error('Missing RIOT_API_KEY in env');
  throw new Error('Missing RIOT_API_KEY');
}

const headers: Record<string, string> = { 'X-Riot-Token': RIOT_KEY };

type Regional = 'americas' | 'europe' | 'asia' | 'sea';

export function regionalForServer(server: string): Regional {
  const s = (server || '').toLowerCase();
  if (['na1', 'br1', 'la1', 'la2'].includes(s)) return 'americas';
  if (['euw1', 'eun1', 'tr1', 'ru1'].includes(s)) return 'europe';
  if (['kr', 'jp1'].includes(s)) return 'asia';
  if (['oc1', 'ph2', 'sg2', 'th2', 'tw2', 'vn2'].includes(s)) return 'sea';
  return 'europe';
}
...
export async function soloQueueEntry(
  ...
  const url = `https://${server}.api.riotgames.com/lol/league/v4/entries/by-summoner/${encodeURIComponent(summonerId)}`;
  const res = await fetch(url, { headers, cache: 'no-store' });
...
export async function summonerByPuuid(server: string, puuid: string): Promise<{ id: string; name: string }> {
  const url = `https://${server}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${encodeURIComponent(puuid)}`;
  const res = await fetch(url, { headers, cache: 'no-store' });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
