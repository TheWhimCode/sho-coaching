...
export async function recentSoloMatchIds(regional: Regional, puuid: string, count: number) {
  ...
  u.searchParams.set("queue","420"); u.searchParams.set("start","0"); u.searchParams.set("count", String(Math.max(1,Math.min(20,count))));
...
export async function leagueEntriesByPuuid(platform: string, puuid: string) {
  return riotFetchJSON<any[]>(`https://${platform}.api.riotgames.com/lol/league/v4/entries/by-puuid/${encodeURIComponent(puuid)}`);
}
export async function leagueEntriesBySummoner(platform: string, summonerId: string) {
  return riotFetchJSON<any[]>(`https://${platform}.api.riotgames.com/lol/league/v4/entries/by-summoner/${encodeURIComponent(summonerId)}`);
}
