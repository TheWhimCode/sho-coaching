...
export default function SessionSummaryCard({ preset, onRetake, onSchedule }: Props) {
  ...
  const scheduleText = "#0A0A0A";

  return (
    <div
      className="relative rounded-2xl p-[2px] shadow-[0_6px_24px_rgba(0,0,0,0.3)]"
      style={{
    ...
    >
      ...
      <div className="flex items-stretch min-h-[12rem] overflow-hidden rounded-2xl">
        ...
        <div className="relative flex-1 min-w-0 bg-white/[.04] backdrop-blur-sm border border-white/10 border-r-0 rounded-l-2xl text-left px-6 md:px-8 py-4 md:py-6">
          {/* readability scrim behind text — slightly inset so the outer border shows more */}
          <div className="pointer-events-none absolute inset-[2px] z-0 rounded-l-2xl bg-gradient-to-r from-black/35 via-black/25 to-transparent" />
            ...
            <div className="mt-auto pt-4 md:pt-5 flex items-center gap-2">
              <button
                ...
                className="rounded-xl px-4 py-2 text-sm font-semibold transition
                           ...
                           transform-gpu duration-150 hover:-translate-y-0.5"
                style={{
              ...
              {onRetake && (
                <button
                  onClick={onRetake}
                  className="rounded-xl px-4 py-2 text-sm border border-white/20 text-white
                             bg-white/[.05] hover:bg-white/[.08] hover:border-white/30
                             transform-gpu duration-150 hover:-translate-y-0.5"
                >
                  Retake survey
                </button>
